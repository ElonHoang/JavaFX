...
public class ProductIndexUIController {
    ...
    @FXML
    private TableView<Product> tvDisplay;

    @FXML
    private TableColumn<Product, Integer> tcQuantity;

    @FXML
    private TableColumn<Product, Integer> tcSum;
    ...
    @FXML
    void btnDateClick(ActionEvent event) throws IOException, InterruptedException, ParseException {
        Nagatice.getInstance().goToDate(p);
    }
    ...
    void btnsearch(ActionEvent event) throws SQLException {
        ObservableList<Product> p = Product.Search(txtSearch.getText());
        if (p != null) {
            tvProduct.setItems(p);
        }
//        Search();
    }
    ...
    void onMouseClick(MouseEvent event) {
        ...
        if (nul != null) {
            Image myimage;
            // if(nul.getImg().contains("/img/")){
            myimage = new Image(getClass().getResourceAsStream(nul.getImg()));
            image.setImage(myimage);
            //} else {
//                       File file = new File(nul.getImg());
